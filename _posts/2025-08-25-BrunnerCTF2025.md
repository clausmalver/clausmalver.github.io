---
author: clausmalver
title: BrunnerCTF25 - Ticket app writeup
description: 
date: 2025-08-25
categories: [CTF]
tags: [CTF, BrunnerCTF]
...
# Tickets App (User) & Tickets App (Root)
Authors:
@c3lphie, @ajtemp, @Ruttimads & @Atlas

## Intro
We all play for the CTF team Jutlandia which is based in Aalborg, Denmark. We are a team consisting of players with diverse backgrounds, ranging from students to those who work with IT on a daily basis. This writeup is for the [BrunnerCTF 2025](https://brunnerctf.dk/) tournament which was held from 22/08/2025 14:00 CEST to 24/08/2025 14:00 C EST with almost 3000 players and 1500 teams participating. Our team placed 15th on the international leaderboard and a 3rd place on the Danish leaderboard - The top 10 Danish teams were eligible for the prizes in this tournament:
...
---
## TLDR
This challenge combines multiple techniques to escalate from an ordinary user on the website, to an initial foothold, and finally to full root access of the box. Following a chain through JWT manipulation → SQL injection → Reverse shell → Reverse engineering → SUID binary abuse. From the reverse shell you were able to obtain the first flag `user.txt` and after exploiting a custom binary on the box you could elevate to root and extract the `/root/root.txt` flag. It was completed very late at night, and we might have overcomplicated some of the steps. Nevertheless, it was a very fun challenge that required participation from multiple team members.

---

## First part of the challenge
When you first launch the instance, you are presented with a webpage, where you are able to secure a ticket to some of the our favourite events! We for sure would have won in a bakeoff competition with our infamous “Othello Lagkage” 👀

![[/assets/img/brunner/Frontpage.png]]

On the site you can either register an account or login with an account. We created an account with the following credentials test:test which allow us to gain access to the dashboard `https://tickets-app-user-3eea27441b01d05c.challs.brunnerne.xyz/dashboard`

When our account is created, the backend generates a JWT token for our account that provide the server with our username and privilege on the website as seen on the screenhot from Firefox
...
If you run SQL map on the search endpoint, it reveals that is it vulnerable for SQL injection attacks.
...
First we enumerated the database which revealed 3 tables we can dig into. The settings is the first on the list, so lets start there and dump the table with the following command:
...
The API key needed to use the upload module functionality is seen below from the table dump from sqlmap.
...
Now that we have the functionality to upload a python file and execute it, we need to generate a payload and upload it to the box:
...
To do the upload we can use this curl command:
...
To be able to catch the reverse shell, you can set up a ngrok instance or whatever tool that is to your liking and then catch it with an listener `nc -lvnp 4444`

![[/assets/img/brunner/revshell.png]]
...
![[/assets/img/brunner/user.txt.png]]
...
@c3lphie was kind enough to write a blazing fast rust binary that we could use:
...
![[/assets/img/brunner/rootflag.png]]
